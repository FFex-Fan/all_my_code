...
import gymnasium as gym  # 用于构建和交互强化学习环境
import numpy as np
import collections  # Python内置数据结构模块，包含 deque（双端队列）等
from tqdm import tqdm  # 显示进度条的库，用于显示训练进度
import torch  # PyTorch库，支持张量计算、自动微分和神经网络训练
...
def test():
    ...
    agent = DQN(state_dim, hidden_dim, action_dim, lr, gamma, epsilon, target_update, device)  # 初始化 DQN 代理
