...
# 定义游戏环境类
class GameEnv:
    def __init__(self, widht=51, hight=21, agent_hidden=15, food_hidden=30):
    ...
    def greedy_polity(self):  # 先不考虑 Agent 朝向问题（改进）
        all_apple_pos = self.get_all_foods()
        my_pos = (self.agent2.x, self.agent2.y)
    ...
    def move(self, agent1_action, agent2_action):
        ...
        if not self.agent1.is_hidden():
            if agent1_action in [0, 1, 2, 3]:  # 移动类动作
                ...
                beam_set = self.agent1_actions[agent1_action](env_x_size=self.size_x, env_y_size=self.size_y)
                self.agent1_beam_set.extend(beam_set)
            else:  # 旋转或停留类动作
                self.agent1_actions[agent1_action]()
        else:
            pass  # 代理隐藏时，不执行动作，但不修改 beam_set
        ...
        if not self.agent2.is_hidden():
            ...
            #     beam_set = self.agent2_actions[agent2_action](env_x_size=self.size_x, env_y_size=self.size_y)
            #     self.agent2_beam_set.extend(beam_set)
            # else:  # 旋转或停留类动作
            #     self.agent2_actions[agent2_action]()
            number = random.random()
            if number < 0.05:
                beam_set = self.agent2.beam(env_x_size=self.size_x, env_y_size=self.size_y)
                self.agent2_beam_set.extend(beam_set)
            elif number < 0.1:
                self.agent2.turn_right()
            elif number < 0.15:
                self.agent2.turn_left()
            elif number < 0.2:
                self.agent2.stay()
            else: # 此处需要修改
                pass
        ...
        for food in self.food_objects:
            ...
            if not food.is_hidden():  # apple 被吃的逻辑
        ...
        # 例如，当所有食物都被吃完时，游戏结束
        if all(food.is_hidden() for food in self.food_objects):
            done = True
    ...
    def step(self, agent1_action, agent2_action):
        # 执行动作，获取奖励
        r1, r2, done = self.move(agent1_action, agent2_action)
