...
from .models import ContestBoardTotal, ContestComingInfo, ContestTutorial, ContestAnnouncement, ContestRatingChange, \
    ContestBoard, ContestComment, ContestInfo, ContestProblem, ContestRegister, StudentChoiceAnswer, \
    ContestChoiceProblem
from problem.models import ChoiceProblem
from .serializers import ContestBoardTotalSerializer, ContestComingInfoSerializer, ContestTutorialSerializer, \
    ContestRatingChangeSerializer, ContestAnnouncementSerializer, ContestBoardSerializer, ContestCommentSerializer, \
    ContestInfoSerializer, ContestProblemSerializer, ContestRegisterSerializer, StudentChoiceAnswerSerializer, \
    ContestChoiceProblemSerializer
# import datetime
...
class CurrentTimeView(APIView):
    def get(self, request):
        ...
        # my_datetime_aware = make_aware(my_datetime)
        return Response(datetime.now(), HTTP_200_OK)
...
class ContestBoardFilterAPIView(APIView):
    ...
    def post(self, request, format=None):
        ...
        for b in boards:
            ...
            if usertype != 3 and userid != b.username and contest.lockboard == 1 and contest.lasttime - (
                    datetime.datetime.fromtimestamp(
                        b.submittime / 1000) - contest.begintime).total_seconds() <= contest.locktime * 60:
                b.type = 2
...
class ContestIsBoardLockAPIView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if contest.lockboard == 1 and contest.lasttime - (
                my_datetime_aware - contest.begintime).total_seconds() <= contest.locktime * 60:
    ...
    def post(self, request, format=None):
        try:
            ...
            for stu_info in stu_cpro_info:
                ...
                StudentChoiceAnswer.objects.filter(contestid=contestid, username=stu_info.username).update(
                    score=stu_score)
