...
# echo $python_loc_name
# echo $python_version


python_folder_name="${python_loc_name:0:24}"lib/${python_version}
...
sed -i '' '84s/tf.multinomial(self.inputs, 1)/tf.multinomial(self.inputs, 1, output_dtype=tf.int32)/' "$python_folder_name"/site-packages/ray/rllib/models/tf/tf_action_dist.py # Same as above
sed -i '' '656i\
        actions = np.array(actions, dtype=policy.action_space.dtype)' "$python_folder_name"/site-packages/ray/rllib/evaluation/sampler.py # Insert action to uint8 conversion to save even more memory

# Apply patch for https://github.com/ray-project/ray/pull/8491 (fixed in ray 0.8.6, remove this when upgrading to ray >= 0.8.6)
sed -i '' '164i\
        return self.sess.run(self.variables)' "$python_folder_name"/site-packages/ray/experimental/tf_utils.py
